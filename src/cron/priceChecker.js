  ...
  try {
    ...
    for (let alert of alerts) {
      try {
        ...
        // توقف قصير بين الطلبات (لتجنب Rate Limiting)
        await sleep(1000);
...
async function processAlert(alert) {
  ...
  try {
    ...
    console.log(`💰 [${alert.id}] السعر الحالي: ${priceData.price} ريال (${priceData.airline})`);
    ...
    if (analysis) {
      ...
      console.log(`✅ [${alert.id}] تم إرسال تنبيه للمستخدم ${alert.name}`);

      return { success: true, notificationSent: true };
    }

    console.log(`ℹ️ [${alert.id}] لا حاجة لإرسال تنبيه (السعر لم يتغير بشكل كبير)`);
